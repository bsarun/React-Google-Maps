...
describe('App', () => {
	beforeEach(() => {
		props = {
			...
			intl: {
				doesMessageExist : jest.fn().mockReturnValue('string'),
				messageIfExists : jest.fn().mockReturnValue('')
			},
			applicationDetails:{
	...
	it('Testing functions', () => {
	  ...
	  it('Testing renderDevToolsContentIfEnvVarSet', () => {
		...
		it('unmount', () => {
			...
			AppDispatcher.unregister = jest.fn().mockReturnValue(true)			
			// if(typeof this.binds.routerUnlistener === "function") {
			// 	this.binds.routerUnlistener();
			// }
